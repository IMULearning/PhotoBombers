...
- (void)setPhoto:(NSDictionary *)photo {
    ...
    /* User id */
    NSString *userId = [_photo valueForKeyPath:@"user.id"];
    ...
    /* Set avatar */
    NSString *avatarKeyPath = @"user.profile_picture";
    NSString *avatarCachePath = [NSString stringWithFormat:@"%@-avatar", userId];
    [THPhotoController imageForPhoto:_photo atKeyPath:avatarKeyPath withCacheKey:avatarCachePath completion:^(UIImage *image) {
        self.avatarImageView.image = image;
    }];
    
    /* Set likes */
    NSNumber *likeCount = self.photo[@"likes"][@"count"];
    [self.likesButton setTitle:[likeCount stringValue] forState:UIControlStateNormal];
    
    /* Set comments */
    NSNumber *commentsCount = [_photo valueForKeyPath:@"comments.count"];
...
- (void)openUser:(id)sender {
    NSString *userName = self.photo[@"user"][@"username"];
    NSURL *userURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://instagram.com/%@", userName]];
